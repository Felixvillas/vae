...
if __name__ == "__main__":
    ...
    for ep in range(epochs):
        ...
        loss_num = 0
        for _ in tqdm(range(max([train_num // batch_size for train_num in train_nums]))):
            ...
            inner_loss_re, inner_loss_kl, outer_loss_re, outer_loss_kl = Vae.update(
                inner_xs=[data_train[index] for data_train, index in zip(datas_train, inner_indexs)],
                outer_xs=[data_train[index] for data_train, index in zip(datas_train, outer_indexs)]
            )
            loss_num += 1
        ...
        for data_name, ilr_i, ilk_i, olr_i, olk_i in zip(
            ...
            print(f"\t\tInnerReLoss: {ilr_i} | \tInnerKlLoss: {ilk_i}")
            print(f"\t\tOuterReLoss: {olr_i} | \tOuterKlLoss: {olk_i}")

        if (ep + 1) % test_interval == 0:
            ep_idx = (ep + 1) // test_interval
            # test
            Vae.eval()
            # z, _, _ = Vae.inner_encode(datas_test)
            # x_re = Vae.inner_decode(z)
            # x_re = [x_re_i.clamp(0., 1.) for x_re_i in x_re]
            ...
            root_path = f'./image/maml/{cat_data_name}/vae/{low_root}/'
            if not os.path.exists(root_path):
                os.makedirs(root_path)

            for data_name, store_x_re_i in zip(data_names, store_x_re):
                torchvision.utils.save_image(store_x_re_i, os.path.join(root_path, f"{data_name}_test_{ep_idx}.png"), nrow=16)

            # generate
            x_gen = Vae.generate(num_sample=test_batch_size).clamp(0., 1.)
            x_gen = x_gen.view(-1, 1, glength, gwidth)
            torchvision.utils.save_image(x_gen, os.path.join(root_path, f"generate_{ep_idx}.png"), nrow=16)